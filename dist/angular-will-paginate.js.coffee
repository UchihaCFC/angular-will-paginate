###
Modified and caffeinized by Carol Fernandez (UchihaCFC)
###
"use strict"
...
.directive "willPaginate", ->
  ...
  controller: [
    ...
    ($scope) ->
      $scope.getPage = (num) ->
        $scope.onClick num  if $scope.onClick
        return
  ]
  ...
  link: ($scope) ->
    ...
    $scope.windowedPageNumbers = ->
      ...
      if windowFrom < 1
        ...
        windowFrom = 1
        windowTo = $scope.params.totalPages  if windowTo > $scope.params.totalPages
    ...
    $scope.$watch "config", (newVal) ->
      return  if typeof newVal is "undefined"
      $scope.options = angular.extend(angular.copy($scope.defaults, {}), angular.copy(newVal, {}))
      $scope.render()  if $scope.params.currentPage
      return
